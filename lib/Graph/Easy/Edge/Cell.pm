...
sub _html_edge_hor
  ...
  if ($e_flags & EDGE_END_W)
    ...
    $rc->[2] = '<td rowspan=2 class="##class## eb">&nbsp;</td>' . "\n" . $rc->[2];
    }
...
sub as_html
  ...
  for my $a (@$code)
    ...
    if (ref($a))
	  ...
	  $border, $border_v, $label_style, $edge_color, $bg, $as, $title, $label, $cl);
	}
      }
    else
      {
...
sub _format_td
  {
  my ($self, $c,
	$border, $border_v, $label_style, $edge_color, $bg, $as, $title, $label, $cl) = @_;
